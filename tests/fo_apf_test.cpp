...
struct fo_apf : public testing::Test {
	FO_APF *m_filter;
...
TEST_F(fo_apf, coeff_calculation) {
	// Assert
	EXPECT_NEAR(m_coeffs.a0, -0.71659, 0.00001);
#ifdef DEBUG_OUTPUT
	debug_print(m_coeffs);
#endif
}

TEST_F(fo_apf, process_0) {
	// Act
...
TEST_F(fo_apf, process_1) {
	// Act
	auto yn = m_filter->process(TEST_SAMPLE_1);
	// Assert
	EXPECT_NEAR(yn, -0.71659, 0.00001);
#ifdef DEBUG_OUTPUT
	std::cout << "yn: " << yn << std::endl;
#endif
}
